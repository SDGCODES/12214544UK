node ('cm-linux') {
	try {
		...
		env.NO_PROXY=".hsbc"
		sh "sh ${env.WORKSPACE}/Environconfig_UK_Terraform/groovy/build_infra_dbproxy.sh uk gmitest destry"
		}
	}
}
		stage('create vm'){
		withCredentials([file(credentialsId: 'hsbc-12214544-provuk-prod-sa-terraform', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
			env.HTTP_PROXY="https//googleapis-dev.gcp.cloud.hk.hsbc:3128"
			env.NO_PROXY=".hsbc"
			sh "sh ${env.WORKSPACE}/Environconfig_UK_Terraform/groovy/build_infra_dbproxy.sh uk gmitest apply"
		}
	}
...
def notifySuccess() {
	print("This job built success,it's the last job so this mail is to notify developers")
	
	emailext (
	subject: "Success Pipeline * ${currentBuild.fullDisplayName}",
	body: "${currentBuild.projectName} is build success. The log: ${env.BUILD_URL}."
	to: 'soumi.dasgupta@noexternalmail.hsbc.com;soumi.dasgupta@notreceivingmail.hsbc.com'
	}
	}